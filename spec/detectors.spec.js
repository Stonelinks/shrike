describe('utils', function() {
  ...
  it('detect native 1d float arrays', function() {
    ...
    // strings, ints, floats, etc are not arrays
    // TODO make this as good as the isNumber stuff
    expect(shrike.isTypedArray(undefined)).not.toBe(true);
    expect(shrike.isTypedArray(false)).not.toBe(true);
    expect(shrike.isTypedArray('0')).not.toBe(true);
    expect(shrike.isTypedArray(0)).not.toBe(true);
    expect(shrike.isTypedArray(1.775)).not.toBe(true);

    // arguments isn't an array!
    expect(shrike.isTypedArray(arguments)).not.toBe(true);

    // this isn't a native 1d flaot array
    expect(shrike.isTypedArray([0, 1, 2])).not.toBe(true);

    // but this is
    var f32a = new Float32Array([100.55]);
    expect(shrike.isTypedArray(f32a)).toBe(true);
  });
